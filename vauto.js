...
function gene() {
	if (gen.value!=0 && gen.value>=1){ 
		if (genty.value==200 || genty.value==500) {
			check();
			if (state) {
...
function load(){
		...
		for(i=0;i<fold.length;i++){
			 if(fold[i][0]==lodput.value && fold[i][1]=="Valid"){
	 		 ...
	 		 if (fold[i][0]==lodput.value && fold[i][1]=="Used"){
	  			loadstat.innerHTML= " Sorry! this voucher has already been used by you";
				 break;
			 }	  
 ...
 function balance(){
	 balval.innerHTML = "Your account balance is N"+bal; 
 }
...
function getvauto(){
 	...
 	if (fill==null){
		...
		fold=JSON.parse(fill);
	};
 	console.log(fold);
	console.log(typeof(fold));
